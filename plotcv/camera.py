...
def create_point(coords, color='red', name='Point', size=6, show_connections=True):
    """
    Create a point with control over connection display
    
    Parameters:
    coords - 3D coordinates of the point (list, tuple or numpy array)
    color - Color for visualization
    name - Name of the point
    size - Size of the point marker
    show_connections - Whether to show connections and coordinates to frames
    
    Returns:
    Dictionary containing point parameters
    """
    import numpy as np

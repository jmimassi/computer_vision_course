...
def create_visualization_3d_2d(cameras, points_3d=None, figure_title="Visualization with 3D and 2D Views", 
                           show_world_frame=True, height=900):
    """
    ...
    for point in points_3d:
        if isinstance(point, (list, tuple, np.ndarray)):
            ...
            processed_points.append({
                ...
                'name': 'Point',
                'size': 6,
                'show_connections': True  # Par défaut, activer les connexions
            })
        elif isinstance(point, dict):
            # Dict format -> ensure all required fields with defaults if not provided
            pt_dict = {
                'coords': np.array(point.get('coords', [0, 0, 0])),
                'color': point.get('color', 'red'),
                'name': point.get('name', 'Point'),
                'size': point.get('size', 6),
                'show_connections': point.get('show_connections', True)  # Utiliser la valeur ou True par défaut
            }
            processed_points.append(pt_dict)
    
    # S'assurer que la propriété show_coordinates est correctement définie pour tous les frames
    for camera in cameras:
        # Par défaut, on utilise la propriété définie dans la caméra, sinon True
        if 'show_coordinates' not in camera:
            camera['show_coordinates'] = True
    ...
    # Add points to 3D plot (tous les points sont visibles)
...
def add_point_frame_connections(fig, frames, points, row=1, col=1, line_color='black', line_width=2):
    """
    ...
    for frame in frames:
        ...
        for point in points:
            # Skip if this point doesn't want connections
            if not point.get('show_connections', True):
                continue
